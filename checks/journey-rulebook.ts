/** Isolated fork chain proof for Circuit cap, pause expiry and stale halt data. */
...
const PORT = 8984
const DAY = 86_400
const outPath = join("scripts", "fork", "out", "journey-rulebook.json")
...
async function main() {
  ...
  try {
    ...
    await send("smallCap", [venue.setCap(programId, { dataAuthority: admin, venue: venueId, symbol,
      capShares: 1n, advShares: 10_000_000_000n, multiplierNum: 1, multiplierDen: 1 })], admin)
    await reject("capReached", 6005, [swapIx()], trader)
    await send("restoreCap", [venue.setCap(programId, { dataAuthority: admin, venue: venueId, symbol,
      capShares: 1_000_000_000n, advShares: 10_000_000_000n, multiplierNum: 1, multiplierDen: 1 })], admin)
    await timeTravelTo(fork.rpcUrl, (await chainNow(fork.rpcUrl)) + 200)
    await reject("haltDataStale", 6002, [swapIx()], trader)
    await send("heartbeatAfterStale", [venue.heartbeat(programId, { relay: admin, venue: venueId, symbol, seq: 2n })], admin)
    ...
    const current = await chain.rpc.getAccountInfo(symbol, { encoding: "base64", commitment: "confirmed" }).send()
    if (!current.value) throw new Error("FWDI symbol missing after breaches")
    const state = Buffer.from(current.value.data[0], "base64")
    const pausedUntil = Number(state.readBigInt64LE(128))
    const breachCount = state[7]
    const breachNow = await chainNow(fork.rpcUrl)
    if (breachCount !== 2 || pausedUntil < breachNow + 91 * DAY || pausedUntil > breachNow + 92 * DAY + 60) {
      throw new Error(`breach state mismatch: count ${breachCount}, pausedUntil ${pausedUntil}, now ${breachNow}`)
    }
    observed.breachCount = breachCount
    observed.pausedUntil = pausedUntil
    await send("heartbeatAfterBreach", [venue.heartbeat(programId, { relay: admin, venue: venueId, symbol, seq: 3n })], admin)
    await reject("paused", 6004, [swapIx()], trader)
    await timeTravelTo(fork.rpcUrl, pausedUntil + 1)
    await send("heartbeatAfterPause", [venue.heartbeat(programId, { relay: admin, venue: venueId, symbol, seq: 4n })], admin)
