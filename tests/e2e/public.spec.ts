...
test("home, explorer and about render live devnet state", async ({ page }) => {
  ...
  await expect(page.locator(`a[href*="${devnet.programId}"]:visible`).first()).toBeVisible()
  ...
  await expect(page.locator(`a[href*="${fork.signatures.fwdiSwap}"]:visible`).first()).toBeVisible()
