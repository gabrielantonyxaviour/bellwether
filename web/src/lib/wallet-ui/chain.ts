...
export async function marketAccounts(pair: Pair, wallet: Address | null, signal?: AbortSignal) {
  ...
  const [venueRaw, symbolRaw, poolRaw, clockRaw] = await Promise.all([
    readAccount(config.venue, signal), readAccount(symbolAddress, signal), readAccount(poolAddress, signal), readAccount(CLOCK_SYSVAR, signal),
  ])
  if (!venueRaw || !symbolRaw || !poolRaw || !clockRaw || clockRaw.length < 40) throw new Error("Venue market or clock accounts are not on this cluster")
  ...
  return {
    ...
    venue, symbol, pool, chainTime, ownerStock, ownerUsdc, stockBalance, usdcBalance, lpAddress,
