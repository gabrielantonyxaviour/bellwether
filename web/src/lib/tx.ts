...
export async function sendTransaction(opts: SendOptions): Promise<TxState> {
  const toastId = toast.loading(`${opts.label} · awaiting signature`, { description: "Approve in your wallet" })
  ...
  try {
    ...
    toast.loading(`${opts.label} · pending`, { id: toastId, description: shortSig(signature), action: view })
    ...
    while (Date.now() < deadline) {
      ...
      if (status && (status.confirmationStatus === "confirmed" || status.confirmationStatus === "finalized")) {
        toast.success(`${opts.label} · confirmed`, { id: toastId, description: shortSig(signature), action: view })
    ...
    toast.error(`${opts.label} · failed`, { id: toastId, description: message, action })
    return set({ phase: "failed", error: message, venueError })
  }
}

/** React state around sendTransaction for one action on a screen. */
export function useTransaction(label: string) {
  ...
  const submit = useCallback(
    (instructions: readonly Instruction[], signer: TransactionSendingSigner) =>
      sendTransaction({ label, instructions, signer, onPhase: setState }),
    [label],
  )
