...
| --- | --- | --- | --- |
| `sc_participant_trade` | **Partial** | Fresh browser wallet signed challenge, was admitted and funded, completed a 0.05 USDC swap through browser `POST /rpc` with the `solana-client` header, and saw its finalized signature on the public tape. The generated Playwright scenario passed all steps, including the SDN fixture refusal and a separate `NotAdmitted` preflight after creating the probe wallet's token accounts. A separate never-admitted wallet also produced a finalized failed swap with `NotAdmitted` (`6000`) on devnet. Mapping revision 2 routes to `/explorer` and `mappingCurrent=true`. | Abel scenario harness proof and Gabriel's Circuit review. |
...
### Browser swap after the RPC CORS fix

After API commit `b135ba6`, public `OPTIONS /rpc` with `Access-Control-Request-Headers: content-type,solana-client` returned `204`, `Access-Control-Allow-Headers: Content-Type,Solana-Client`, and `Access-Control-Max-Age: 600`. A fresh headless Chromium wallet completed the onboard UI, signed a 0.05 USDC buy in `/app/trade/BWRS`, and displayed [swap `K6FPEJymHUmieZkBhNVYEHM6ThFY7xqvBccTkvs5ENQBzRM3xbW9335xgr51mX8ifJPzTPHVj8SZKhrZAGECdyd`](https://solscan.io/tx/K6FPEJymHUmieZkBhNVYEHM6ThFY7xqvBccTkvs5ENQBzRM3xbW9335xgr51mX8ifJPzTPHVj8SZKhrZAGECdyd?cluster=devnet). The public RPC reported finalized, `err: null`, slot `504066268`; the tape row at `2026-09-25T16:05:29Z` reported buy, 0.050000 USDC and 0.005399 shares. **The test wallet's send bridge used direct public devnet RPC for this swap**, so this result does not prove browser `sendTransaction` through `/rpc`. The generated test failed afterward in its separate direct no-credential probe (`InvalidAccount` due missing token accounts); its probe setup and send bridge are corrected for the next run.
